...
這題接續上題，data input從8 bit變9 bit，所以FSM要改。然後有個細節要注意，就是parity的reset等到start階段時也要reset。至於說如何判別```is_start```，這裡得用```assign is_start = (next_state == 4'd1);```，不能用```assign is_start = (state == 4'd1);```的原因在於，會晚一個cycle。
```verilog
module top_module(
    input clk,
    input in,
...
這題應該是這網站上第一題明確指出需使用mealy machine的題目，不過這題跟```Serial receiver with parity checking```這題很像，assign的部分和狀態有關，只要把握一個大原則，assign若和狀態有關，就用next_state判別。像這題原本應該要寫成```assign z = (next_state == three_bit) && x;```，或是```assign z = (next_state == three_bit);```，只是因為題目要求只要3個狀態，所以three_bit的狀態和one_bit的狀態可以合併。
```verilog
module top_module (
    input clk,
