...
這題應該是這網站上第一題明確指出需使用mealy machine的題目，不過這題跟```Serial receiver with parity checking```這題很像，assign的部分和狀態有關，只要把握一個大原則，assign若同時與狀態和輸入有關，就用next_state判別。像這題原本應該要寫成```assign z = (next_state == three_bit) && x;```，只是因為題目要求只要3個狀態，所以three_bit的狀態和one_bit的狀態可以合併。
```verilog
module top_module (
    input clk,
